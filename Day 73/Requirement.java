/*  our task is to assign non-negative numbers smaller than 10 to each variable and then calculate the number of different assignments satisfying all requirements.
Two assignments are different if and only if at least one variable is assigned to a different number in both assignments. 
Print your answer modulo 10^3+7.

Input Format -

The first line contains 2 space-separated integers, n and m, respectively.
Each of the m subsequent lines contains 2 space-seperated integers describing the respective x and y values for an (x<=y) requirement.
